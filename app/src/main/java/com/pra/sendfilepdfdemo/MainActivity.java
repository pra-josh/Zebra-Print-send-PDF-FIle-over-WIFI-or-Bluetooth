...
public class MainActivity extends ConnectionScreen implements GrantedAllPermission {
    ...
    private void connectAndGetData() {
        if (isBluetoothSelected() == false) {
            ...
            multiplePermission.add(Manifest.permission.CHANGE_WIFI_MULTICAST_STATE);
            multiplePermission.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
            multiplePermission.add(Manifest.permission.CAMERA);
            UIHelper.requestingMultiplePermission(false, MainActivity.this,
                   multiplePermission , this);
        } else {
            List<String> multiplePermission = new ArrayList<>();
            multiplePermission.add(Manifest.permission.BLUETOOTH);
            multiplePermission.add(Manifest.permission.BLUETOOTH_ADMIN);
            multiplePermission.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
            multiplePermission.add(Manifest.permission.CAMERA);
            UIHelper.requestingMultiplePermission(true, MainActivity.this,
                    multiplePermission, this);
        }
    }
    ...
    @Override
    public void onGrantedAllPermission(boolean isBluetooth) {
        if (isBluetooth) {
